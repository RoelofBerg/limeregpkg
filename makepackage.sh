...
#Copy the tar distro (made by cd liblimereg && ./autogen && ./configure && make dist)
cp ../liblimereg/liblimereg-0.9.3.tar.gz .

#Import the tar distribution to a new git upstream folder
mkdir liblimereg
pushd liblimereg
git init
git-import-orig -u 0.9.3 ../liblimereg-0.9.3.tar.gz
...
echo of conduct
echo dput ppa:roelofberg/limereg liblimereg0_0.9.3-0ubuntu1_source.changes
